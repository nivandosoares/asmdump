...
| Lane 4: Bank API contracts (30/10/11) | active | entry points identified ($0A:0000, $0A:5322, $0A:8759, $1E:4112, etc.) |

## Lane 1 and Lane 4 Focus (`2026-03-29`)

### Lane 1 — brief analysis, next steps, and checkpoints (2026-03-30)

Brief analysis:
- tools/out/bank30_chunk_registry.json currently reports 6 entries with status "decode-fail" (status_counts: {"decode-fail": 6}). The l001210 probe matrix (tools/out/l001210_probe_matrix/matrix_summary.json) shows runtime hits for several table-confirmed 26FB entries (DF6C, E039, E73F, E800) but zero runtime hits for the unresolved P0/P1 candidates 1E:DA96 (67FB) and 1E:EE7F (26FB) in the sampled scenarios.
- The decompressor implementation tools/decompress_td2_chunk.py contains unreplaced placeholder tokens ("***") in the 42FB and 67FB handlers. These placeholders prevent successful automated decoding and explain many of the "decode-fail" registry entries.

Immediate next steps (actionable, small):
1. Fix the placeholder tokens in tools/decompress_td2_chunk.py (complete the token read/loop logic in the 42FB and 67FB handlers) and add a small smoke unit test against an existing known-good sample (e.g., bank7 42FB or the DA96 sample if available under tools/out).
2. Re-run: make -C tools bank30-validate && make -C tools bank30-registry to regenerate validation and registry artifacts; confirm decode-fail count drops.
3. Re-run the deterministic probe matrix (make -C tools l001210-probe-matrix) using the available MESEN_BIN environment; if DA96/EE7F still show zero hits, broaden probe scenarios to include menu-driven and B1F9 prologue sequences (the repo already contains a B1F9 forced-lane harness).
4. If decompression succeeds for DA96/EE7F, extract nested 26FB windows and run decode_26fb to produce final binaries and publish them under tools/out; update the registry and mark entries resolved.

Checkpoints (evidence gates):
- CP-A: Decoder placeholders removed; tools/decompress_td2_chunk.py passes smoke unit tests (add tools/tests/test_decompress_smoke.py).
- CP-B: Rebuilt registry (tools/out/bank30_chunk_registry.json) shows "decode-fail" count reduced (target: <=2) and P0/P1 are downgraded or resolved.
- CP-C: At least one runtime hit recorded for a previously-unseen candidate (DA96 or EE7F) in tools/out/l001210_probe_matrix/* and reflected in the registry summary.
- CP-D: Final artifacts: decompressed binaries under tools/out/bank30_* and updated markdown report tools/out/bank30_chunk_registry.md committed.

Estimated effort: 2–6 hours for the decoder fixes and local validation; additional time if probe scenarios require expansion or CI variability.




- **Lane 1 (Bank 30 Compression):**
  - Confirmed `1E:DA96` as a major `67FB` chunk (28620 bytes output).
  - Decompressed `tools/out/bank30_67fb_da96.bin` successfully.
  - Verified that `1E:EE7F` is a nested `26FB` chunk within the `DA96` window.
- **Lane 4 (Bank API Contracts):**
  - Mapped primary cross-bank entry points:
    - `Bank 6 -> $0A:0000` (Init?)
    - `Bank 7 -> $0A:5322`, `$0A:8759`
    - `Bank 10 -> $1E:EE22`, `$1E:4112`
  - Confirmed Bank 11 has zero `RTL` instructions, suggesting a non-standard far-call interface or internal-only usage.
- **Bank 2 Archaeology:**
  - Confirmed `L0108EF` in Bank 2 tracks Car 1/2 X-coordinates from `$11F1/$11F3` and updates OAM radar slots `$072C/$0730`.
- **Validation:**
  - `make game.smc` now produces a valid 1MB ROM using the new `hdr.asm`.

## Archaeology Workbench and Live Injection (`2026-03-28`)

- New tooling:
  - `tools/archaeology_workbench.lua`: Mesen-S/Mesen 2 Lua script with live HUD and console logging for gameplay analysis.
  - `tools/analyze_wram_delta.py`: Python tool for comparing specific WRAM addresses between two extraction directories.
- Updated tooling:
  - `tools/mesen_ppu_extract/Program.cs`: Now also dumps `wram.bin` for full state analysis.
- Validation:
  - Successfully analyzed the manual gameplay savestate pair: `lane3_live_race_mid.mss` and `lane3_live_race_plus30f.mss`.
  - Confirmed that `plus30f` is a later gameplay frame (+840 frames relative to `mid_race`).
- Findings:
  - **Mid-Race Snapshot (Frame 4500):** Speed: 190 | Car 1 X: 776 | Car 2 X: 476 | Car 3 X: 582.
  - **Post-Delta Snapshot (Frame 5340):** Speed: 77 | Car 1 X: 888 | Car 2 X: 627 | Car 3 X: 582.
  - Verified that Car 1 and Car 2 are dynamic actors with changing X coordinates, while Car 3 remained static in this window.
  - Confirmed that both frames share the `$9016` gameplay callback family.
- Practical implication:
  - The `mesen_ppu_extract` tool now provides a complete state snapshot (VRAM/CGRAM/OAM/WRAM), enabling deep delta analysis of gameplay mechanics.
  - The confirmed actor movement proves that `$11F1` and `$11F3` are indeed tracking active rivals or traffic.
