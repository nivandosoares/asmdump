...
| Lane 3: Gameplay-era frame archaeology | active | the older promoted `v2_current` lane still explains the screenshot-vs-end-frame split and the queue-cursor equalization path through frames `90..92`, and the new seed-surface audit proves the current `game_11.mss` savestate is a `front_end_menu_seed`, not verified gameplay. The next bounded power-on follow-up is stronger: the default-rival post-`02:9016` corridor now responds to late `A/B`, with both inputs flipping `state_0960` from frame `2050`, `A` opening additional DP/HUD deltas and visible differences from `2052..`, and `B` staying much sparser, so the active question is no longer “does this corridor react at all?” but whether that responsive surface is true gameplay or still presentation-state reuse. |
