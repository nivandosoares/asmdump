...
| Lane 3: Gameplay-era frame archaeology | active | the older promoted `v2_current` lane still explains the screenshot-vs-end-frame split and the queue-cursor equalization path through frames `90..92`, but the new seed-surface audit now proves the current `game_11.mss` savestate is a `front_end_menu_seed`, not verified gameplay: `a_hold` and `b_hold` are pixel-identical dynamic lanes from frame `62`, `a+b` first diverges at frame `219`, and that whole family is now demoted to menu/presentation evidence until a true gameplay seed is recovered |
...
- next defensible target:
  - first recover a verified gameplay seed or deterministic input program that
    lands outside the top-menu selector family, then rerun the bounded sweep
    and visible-phase probes on that recovered seed
  - only after that, decide whether the best next gameplay-facing target is
    the old frame-`91` burst / frame-`92` reset path or a new branch on the
    recovered seed
