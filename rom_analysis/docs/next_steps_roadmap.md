...
- Remaining user-guided heuristic:
  ...
  - no-force timed-input probes now already recover the default top-left rival
    ...
    `active_main = 02:9016`
  - the remaining proving target is now narrower:
    bounded `v3/v4` follow-ups now show absolute-frame windows drift with the
    callback itself, so the next probe should overlap or trigger the
    fourth-slot move relative to live `01:C1D2` rather than sliding fixed
    frame numbers forward by guesswork
