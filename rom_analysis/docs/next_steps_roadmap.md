...
- Remaining user-guided heuristic:
  ...
  - callback-relative `v5/v6` probes now also move the fourth slot organically
    inside live `01:C1D2`:
    `$1C70 = 3`, `$1C76 = 0`, `L00BE76` at `1642`, then sampled
    `active_main = 01:BE43` through the later long window
  - the remaining proving target is now the later no-opponent confirm path:
    pair the callback-relative fourth-slot move with a later `start` confirm
    after `01:BE43` is live instead of spending more runs on slot selection
    itself
