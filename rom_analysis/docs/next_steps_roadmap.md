        ...
        - practical reading:
          ...
          - the remaining caution is operational, not semantic:
            - use `--sequence /dev/null` for isolated `--snes-bg-*` runtime
              ...
              the requested scene
          - the next concrete step is now export-side:
            - explain what `layers/bg1_visible.ppm` actually means in the
              `mesen_ppu_extract` path
            - compare that surface against viewer-style layer exports instead of
              expecting one more local `Mode 7` tweak to close it
