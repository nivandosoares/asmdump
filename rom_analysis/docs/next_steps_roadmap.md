...
- Code-backed split:
  ...
  - the downstream `$0202` three-slot corridor is only reached later from
    `L008B31 -> L008B3E`
- User-supplied menu reading, still unverified as ROM text:
  - the top menu should show road/background plus three signboard options:
    `Game Options`, `Play Test Drive`, `High Scores`
  - the configuration-menu string surface is a useful anchor if a real path
    to the initial menu needs to be re-established
