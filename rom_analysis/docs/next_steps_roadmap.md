...
| 3. Gameplay-frame expansion | in progress | Resume from `rom_analysis/docs/gameplay_default_rival_next_agent_handoff.md`. The older promoted `v2_current` lane still explains the screenshot-vs-end-frame split and the historical queue-cursor equalization path through frames `90..92`, but a fresh current-seed recheck now proves today's `game_11.mss` no longer reproduces that window: the savestate has drifted again since the earlier `v3_ab_compare` fingerprint, `target_frame=90/91` scanline reruns stay flat on `00:8029/00:835F` with `dp_0053/0054/0055/0056 = 0x30/0x30/0x28/0x12`, and `0` write hits land on `00:0053..0056`. The seed audit still proves the current `game_11.mss` is a `front_end_menu_seed`, not verified gameplay. The repo now also has a preserved manual live-race seed pair from the user's Mesen slots `#1/#3`, copied into `manual_artifacts/lane3/`; user context identifies them as Porsche-cockpit Desert Blast gameplay approaching a green NPC car. Those seeds still load onto the same `02:9016/01:96A0/02:8F3C` callback surface and inherited selector family, but differ stably in `state_11f3`, `state_09a2`, and `dp_0053/0054`, so the next gate is no longer “find a gameplay-looking frame” but explain why visually live-race seeds still present as the old `02:9016` family. Current caveat: the headless screenshot path emitted zero-byte PNGs on the new seeds, so the JSON/state side is the trusted evidence surface for now. |
