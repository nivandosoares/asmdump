...
- `build_docs_wiki_report.py`: builds a simple wiki-style HTML index for a curated set of repo markdown docs, with explicit section splits such as attract vs front-end/menu vs gameplay so human review does not mix surfaces accidentally; it now also emits a synced markdown bundle for NotebookLM/offline LLM review, mirrors a published copy into `tools/out/docs_wiki/notebooklm_bundle/`, and keeps per-doc raw links inside that same published root so static-hosted wiki deployments do not 404 when they expose only `docs_wiki/`
...
- `make -C tools docs-wiki`
  ...
  - also mirrors a publish-ready copy under `tools/out/docs_wiki/notebooklm_bundle/` so hosts that publish only the wiki root still serve the NotebookLM markdown targets
