...
- `build_docs_wiki_report.py`: builds a simple wiki-style HTML index for a curated set of repo markdown docs, with explicit section splits such as attract vs front-end/menu vs gameplay so human review does not mix surfaces accidentally
...
- `build_gameplay_frame_bundle.py`: packages one raw gameplay frame dump into a self-contained review bundle with screenshot, isolated `BG1/BG2/OBJ` renders, raw memory, and a lightweight design pack so later gameplay moments can be reviewed without reopening the emulator capture pipeline
- `build_gameplay_bundle_compare.py`: compares two gameplay frame bundles and emits a compact JSON/Markdown delta report so lane 3 can separate road/world persistence from overlay/HUD/OBJ changes
